...
// For now, we don't have "real" functions, just the main() function that is the main program, with
// a hardcoded profile and final 'return 0'.
function:
	INTTYPE ID OPAR CPAR OBRACE vardecl_l block RETURN INT SCOL CBRACE # funcDef;

vardecl_l: vardecl* # varDeclList;

vardecl: typee id_l SCOL # varDecl;

id_l: ID # idListBase | ID COM id_l # idList;
...
if_stat:
	IF OPAR expr CPAR then_block = stat_block (
		ELSE else_block = stat_block
	)? # ifStat;
...
index_mutation:
	...
	| ID myop = (PLUS | MINUS) ASSIGN expr	# indexOperatorAssignment
	| ID PLUS PLUS							# indexPlusPlus
	| ID MINUS MINUS						# indexMinusMinus;
